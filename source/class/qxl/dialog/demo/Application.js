...
qx.Class.define("qxl.dialog.demo.Application", {
  ...
  */
  ...
  members: {
    ...
    _replaceOwnedObject(owner, obj, id = "dialog") {
      try {
    ...
    createAlert(caption, button) {
      let dlg = qxl.dialog.Dialog.alert("Hello World!").set({ caption });

      // next line is for automated UI tests only, not needed for "normal" usage
      this._replaceOwnedObject(button, dlg);
    },

    createWarning(caption, button) {
    ...
    createError(caption, button) {
      let dlg = qxl.dialog.Dialog.error("This is an error!").set({ caption });
      this._replaceOwnedObject(button, dlg);
    },

    createConfirm(caption, button) {
     ...
     * @param button
     */
    createDialogChain(caption, button) {
      let dlg1 = qxl.dialog.Dialog.alert(
      ...
      dlg1
        .promise()
        .then(() => {
          let dlg2 = qxl.dialog.Dialog.confirm(
        ...
        .then((result) => {
          let dlg3 = qxl.dialog.Dialog.confirm(
            "You really " +
              (result ? "" : "don't ") +
        ...
        .then((result) => {
          let dlg4 = qxl.dialog.Dialog.alert(
            result ? "I tell you a secret: It doesn't exist." : "Good to know."
     ...
     * @param button
     */
    createSelect(caption, button) {
      let dlg1 = qxl.dialog.Dialog.select("Select an option:").set({
        caption: caption,
        options: [
      ...
      this._replaceOwnedObject(button, dlg1, "dialog1");
      dlg1.promise().then((result) => {
        let dlg2 = qxl.dialog.Dialog.alert(
          "You selected: '" + result + "'"
        ).set({ caption: caption + " 2" });
        this._replaceOwnedObject(button, dlg2, "dialog2");
        return dlg2.promise();
    ...
    createFormEmbedded(caption, button) {
      let formData = {
        username: {
          type: "TextField",
          label: "User Name",
    ...
    createTabbedFormEmbedded(caption, button) {
      let formData = {
        domain: {
          type: "SelectBox",
          label: "Domain",
      ...
      let form = (this.__embeddedForm = new qxl.dialog.FormEmbed({
        ...
        setupFormRendererFunction(form) {
          ...
          renderer = new qxl.dialog.TabbedMultiColumnFormRenderer(
            form,
            tabInfo
          );

          // We want to know when a tab switch occurs
          renderer.addListener("changeTab", (e) => {
